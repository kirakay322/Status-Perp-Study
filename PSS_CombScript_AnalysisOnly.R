...
## Sample dummy codes
d4$Sample_DC1 <- as.numeric(dplyr::recode(d4$Sample, "0" = "1", "1" = "-1", 
                                          "2" = "0"))
d4$Sample_DC2 <- as.numeric(dplyr::recode(d4$Sample, "0" = "1", "1" = "1", 
                                          "2" = "-2"))
...
# Step 2: IVs of interest - SONA IS A PREDICTOR

RQ1A_Y2_step2 <- lm(StigmaP_av ~ Sample_DC1 + Sample_DC2 + ESJ_av_c + 
                      Atonement_fact_r, data = d4_hi)
...
# Step 2: IVs of interest - SONA and Prolific 2 both different
